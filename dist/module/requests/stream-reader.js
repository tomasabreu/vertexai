...
import { createEnhancedContentResponse } from "./response-helpers.js";
 ...
 */
export function getResponseStream(inputStream) {
    ...
    start(controller) {
      ...
      function pump() {
        return reader.read().then(({
          ...
          if (done) {
            if (currentText.trim()) {
              controller.error(new VertexAIError(VertexAIErrorCode.PARSE_FAILED, 'Failed to parse stream'));
              return;
            }
            controller.close();
            return;
          ...
          while (match) {
            try {
              ...
              controller.error(new VertexAIError(VertexAIErrorCode.PARSE_FAILED, `Error parsing JSON response: "${match[1]}`));
              return;
            }
